...
import { Filters, SortDataOption, SortOn, SortMethod, SortOrientation, SortField } from "./components/App";
...
export async function getRangeData(rangeAddress?: string): Promise<{ address: string; values: any[][] }> {
  return await Excel.run(async (context) => {
    try {
      ...
      // Load necessary properties using the RangeValueType enum
...
export async function filterDataOperation(
  ...
  return await Excel.run(async (context) => {
    ...
    let columnIndex = column.charCodeAt(0) - 65; // Convert column letter to index (A=0, B=1, etc.)
